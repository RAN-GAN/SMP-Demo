...
export const mockAPI = {
  ...
  async getStudentById(studentId) {
    await simulateDelay();
    const student = fakeData.students.students.find(s => s.id === parseInt(studentId));
  ...
  async getAttendance(studentId, courseId) {
    ...
    if (studentId) {
      attendance = attendance.filter(a => a.student_id === parseInt(studentId));
    }
    if (courseId) {
  ...
  async updateGrade(gradeData) {
    await simulateDelay();
    const gradeIndex = fakeData.grades.grades.findIndex(g => g.id === gradeData.id);
    if (gradeIndex !== -1) {
      fakeData.grades.grades[gradeIndex] = { ...fakeData.grades.grades[gradeIndex], ...gradeData };
    }
  ...
  async getCalendar(year, month) {
    ...
    if (year && month) {
      const { generateCalendarEvents } = await import('../utils/fakeDataGenerator.js');
  ...
  async getProfile() {
    await simulateDelay();
    // Return the first faculty member as the logged-in user
    const profile = fakeData.faculty.faculty[0];
    return {
      id: profile.id,
      name: profile.name,
      email: profile.email,
      ...
      courses_teaching: profile.courses_teaching,
      photo_url: profile.photo_url,
      bio: "Experienced faculty member dedicated to student success and academic excellence.",
      achievements: [
      ...
      social_links: {
        ...
        google_scholar: "https://scholar.google.com/sample",
        researchgate: "https://researchgate.net/profile/sample"
      }
    };
  },
    ...
    const mockNews = [
      {
        ...
        description: "Researchers develop new quantum computing architecture that could revolutionize computational capabilities...",
      ...
      },
      {
        ...
        description: "Machine learning algorithms are accelerating pharmaceutical research and drug development processes...",
      ...
      },
      {
        ...
        description: "New technologies are making data centers more energy-efficient and environmentally friendly...",
      ...
      },
      {
        ...
        description: "Latest developments in deep learning are pushing the boundaries of what's possible in image recognition...",
      ...
      },
      {
        ...
        description: "How blockchain is transforming supply chain transparency and traceability across industries...",
      ...
      },
      {
        ...
        description: "Addressing security challenges as Internet of Things devices become increasingly prevalent...",
    ...
    const mockHackathons = [
      {
        ...
        description: "Join us for an exciting 48-hour hackathon where you'll build innovative solutions using the latest technologies."
      },
      {
        id: 2,
      ...
      },
      {
        ...
        themes: [{ name: "Blockchain" }, { name: "Payments" }, { name: "Banking" }],
        prizes: [{ title: "Winner", amount: "$7,500" }],
        registrations_count: 650,
        description: "Transform the future of finance with innovative solutions."
      }
    ];
